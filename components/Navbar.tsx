...
export default function Navbar() {
  ...
  const [language, setLanguage] = useState<'BN' | 'EN'>('BN'); // Language State
  ...
  const toggleLanguage = () => {
    ...
    // Note: To fully implement translation, you will need a library like next-intl or i18next
  };
    ...
    <nav className="bg-purple-900 text-white shadow-lg sticky top-0 z-[100]">
      <div className="max-w-7xl mx-auto px-4 py-3 flex justify-between items-center">
        ...
        <ul className="hidden xl:flex items-center gap-4 font-medium text-sm">
          ...
          <li><Link href="/activities" className="hover:text-purple-300 transition">ক্রিয়া</Link></li>
          <li><Link href="/digital-activities" className="hover:text-purple-300 transition">ডিজিটাল কার্যক্রম</Link></li>
          ...
          <li><Link href="/pay-fees" className="bg-amber-500 text-purple-900 px-3 py-1 rounded-full hover:bg-amber-400 font-bold transition">বেতন দিন</Link></li>
      ...
      <div className={`fixed top-0 right-0 h-full w-64 bg-purple-950 shadow-2xl z-[201] p-6 transform transition-transform duration-300 ease-in-out overflow-y-auto ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <ul className="flex flex-col gap-5 text-base font-semibold" onClick={closeMenu}>
          ...
          <li><Link href="/results-activities" className="flex items-center gap-3"><span className="w-1.5 h-1.5 bg-purple-400 rounded-full"/> রেজাল্ট</Link></li>
          <li><Link href="/activities" className="flex items-center gap-3"><span className="w-1.5 h-1.5 bg-purple-400 rounded-full"/> ক্রিয়া</Link></li>
          <li><Link href="/digital-activities" className="flex items-center gap-3"><span className="w-1.5 h-1.5 bg-purple-400 rounded-full"/> ডিজিটাল কার্যক্রম</Link></li>
          ...
          <li><Link href="/pay-fees" className="flex items-center gap-3 text-amber-400"><span className="w-1.5 h-1.5 bg-amber-400 rounded-full"/> বেতন দিন</Link></li>
          ...
          <li className="pt-2 border-t border-purple-800">
            <button onClick={(e) => { e.stopPropagation(); toggleLanguage(); }} className="flex items-center gap-2 text-purple-300 bg-purple-800/50 px-4 py-2 rounded-lg w-full">
              <FaGlobe /> ভাষা পরিবর্তন ({language === 'BN' ? 'English' : 'বাংলা'})
            </button>
          </li>
